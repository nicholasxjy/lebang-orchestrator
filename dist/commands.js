...
export async function executeCommand(argv, options = {}) {
    ...
    try {
        ...
        if (!parsed.command)
            throw new EngineError("a command is required");
        ...
        if (parsed.command === "task") {
            ...
            if (!task)
                throw new SchemaError(`unknown task: ${parsed.values[0]}`);
        ...
        const configPath = resolveConfigPath(repo, parsed.config === undefined ? undefined : resolve(cwd, parsed.config));
...
function parseArguments(argv) {
    const parsed = { values: [], help: false };
    for (let index = 0; index < argv.length; index += 1) {
        ...
        if (value === "--repo" || value === "--config") {
            ...
            if (!next)
                throw new EngineError(`${value} requires a path`);
            if (value === "--repo")
                parsed.repo = next;
            else
                parsed.config = next;
        ...
        if (!parsed.command) {
            if (!commands.has(value))
                throw new EngineError(`unknown command: ${value}`);
...
function printStatus(store) {
    ...
    lines.push(`summary ${[...counts].sort(([left], [right]) => left.localeCompare(right)).map(([key, count]) => `${key}=${count}`).join(" ")}`);
    return `${lines.join("\n")}\n`;
}
function printGraph(store) {
...
function printLogs(store, taskId) {
    if (!existsSync(store.logsDir))
        throw new StoreError(`no logs found for task ${taskId}`);
