...
import { coderPrompt, integratorPrompt, plannerPrompt, replanPrompt, reviewerPrompt, testerPrompt, } from "./prompts.js";
import { AgentRunError, PiRunner } from "./runner.js";
...
export class OrchestratorEngine {
    ...
    async run() {
        if (!existsSync(join(this.store.root, "plan.json")) ||
        ...
        for (;;) {
            ...
            if (ready.length > 0) {
                ...
                for (const task of ready.sort((left, right) => left.id.localeCompare(right.id))) {
                    if (task.assignedAgent !== null && owners.has(task.assignedAgent))
                        continue;
                    batch.push(task);
                    if (task.assignedAgent !== null)
                        owners.add(task.assignedAgent);
                    if (batch.length >= this.config.maxWorkers)
                        break;
                }
    ...
    async executeTask(taskId) {
        await this.store.withTaskLock(taskId, async () => {
            ...
            if (task.status !== "ready")
                throw new EngineError(`task ${task.id} is not ready: ${task.status}`);
            if (task.assignedAgent === null)
                throw new EngineError(`task ${task.id} has no assigned agent`);
            ...
            try {
                ...
                if (AllowedTransitions[current.status].has("failed")) {
                    transitionTask(this.store, current, "failed", current.assignedAgent ?? plannerIdentity, reason);
                }
                else {
    ...
    async testAndReview(plan, task, worktree, coder, initialCoderResult) {
        ...
        while (task.status === "testing") {
            ...
            if (review.status === "approved") {
                transitionTask(this.store, task, "approved", agentForRole(this.config, "reviewer").identity, "review approved");
                return;
            }
            task.reviewAttempts += 1;
            this.store.saveTask(task);
            if (task.reviewAttempts >= this.config.maxReviewAttempts) {
                transitionTask(this.store, task, "blocked", agentForRole(this.config, "planner").identity, "review attempt limit reached");
                return;
            }
            ...
            transitionTask(this.store, task, "changes_requested", agentForRole(this.config, "reviewer").identity, "review requested task-level changes");
            ...
            if (testOnly) {
                ...
                transitionTask(this.store, task, "testing", tester.identity, "tester rework ready for independent execution");
                continue;
            }
            transitionTask(this.store, task, "reworking", coder.identity, "returned to original coder");
            coderResult = await this.runCoder(plan, task, worktree, coder, review.issues.map((issue) => ({ ...issue })));
        }
    }
    async runCoder(plan, task, worktree, coder, reworkIssues) {
        const dependencies = Object.fromEntries(plan.tasks
        ...
        const [result] = await this.runner.run({
            ...
            taskId: task.id,
            cwd: worktree,
            prompt: coderPrompt(plan.goal, task, worktree, dependencies, reworkIssues, coder.identity, coder.role),
        ...
        if (result.status !== "completed") {
            transitionTask(this.store, task, result.status === "blocked" ? "blocked" : "failed", coder.identity, result.summary || result.blockers.join("; "));
            return result;
        }
    ...
    async acceptCoderEvidence(task, worktree, result) {
        if (result.taskId !== task.id) {
            throw new EngineError(`coder returned taskId ${result.taskId}, expected ${task.id}`);
        }
        if (result.status !== "completed")
            throw new EngineError("coder evidence is not completed");
    ...
    async runTester(plan, task, worktree, coderResult) {
        const tester = agentForRole(this.config, "tester");
        const before = task.commit;
        if (before === null)
            throw new EngineError(`task ${task.id} has no coder commit`);
        const [result] = await this.runner.run({
            agent: tester,
            taskId: task.id,
            cwd: worktree,
        ...
        transitionTask(this.store, task, "reviewing", tester.identity, `independent testing ${result.status}`);
        return result;
    }
    async runReviewer(task, worktree, coderResult, testResult) {
    ...
    async resume() {
        ...
        let state = this.store.loadState();
        const plannerIdentity = agentForRole(this.config, "planner").identity;
        if (plan.tasks.some((task) => task.status === "integrating") ||
        ...
        for (const snapshot of plan.tasks) {
            ...
            await this.store.withTaskLock(snapshot.id, async () => {
                ...
                if (AllowedTransitions[task.status].has("interrupted")) {
                    transitionTask(this.store, task, "interrupted", plannerIdentity, `recovering task left in ${originalStatus}`);
                }
                if (!["running", "self_verifying", "testing"].includes(originalStatus))
                    return;
                ...
                if (task.assignedAgent === null)
                    return;
                await this.testAndReview(currentPlan, task, task.worktree, this.config.agents[task.assignedAgent], coderResult);
            });
        }
    ...
    async retry(taskId) {
        ...
        const unsatisfied = task.dependencies.filter((dependency) => !["approved", "completed"].includes(byId.get(dependency).status));
        ...
        if (coderResult && task.worktree !== null) {
            ...
            if (task.status === "failed" || task.status === "blocked") {
                transitionTask(this.store, task, "reworking", tester.identity, "retry recovered committed coder evidence");
            }
            transitionTask(this.store, task, "testing", tester.identity, "retry resumed independent testing");
            if (task.assignedAgent === null)
                throw new EngineError(`task ${task.id} has no assigned agent`);
            try {
                await this.testAndReview(plan, task, task.worktree, this.config.agents[task.assignedAgent], coderResult);
            }
            catch (error) {
        ...
        transitionTask(this.store, task, "ready", task.assignedAgent ?? agentForRole(this.config, "planner").identity, "explicit retry requested");
    }
    async reviewTask(taskId) {
        ...
        transitionTask(this.store, task, "interrupted", agentForRole(this.config, "planner").identity, "explicit review requested");
        transitionTask(this.store, task, "testing", agentForRole(this.config, "tester").identity, "refresh independent test evidence before review");
        if (task.assignedAgent === null)
            throw new EngineError(`task ${task.id} has no assigned agent`);
        await this.testAndReview(plan, task, task.worktree, this.config.agents[task.assignedAgent], coderResult);
    }
    async integrate() {
        ...
        for (const task of activeTasks) {
            if (task.status === "approved") {
                transitionTask(this.store, task, "integrating", integrator.identity, "approved task selected for integration");
            }
        }
        ...
        let commits;
        try {
            ({ path: integrationPath, commits } = await this.git.prepareIntegrationWorktree(plan.tasks, plan.baseCommit, state.runId, integrator.identity, new Set(state.integratedCommits)));
        }
        catch (error) {
            if (error instanceof GitError)
                return this.integrationFailure(plan, integrator.identity, error);
            throw error;
        }
        state.status = "final_validating";
        ...
        try {
            [result] = await this.runner.run({
                ...
                taskId: "INTEGRATION",
                cwd: integrationPath,
                prompt: integratorPrompt(plan.goal, plan, commits, validations, state.integrationBranch),
        ...
        if (result.status === "completed") {
            for (const task of this.store.loadPlan().tasks) {
                if (task.status === "integrating") {
                    transitionTask(this.store, task, "completed", integrator.identity, "repository-wide validation passed");
                }
            }
        }
        return result;
    }
    integrationFailure(plan, integratorIdentity, error, commits = [], validations = []) {
        const issue = {
            severity: "high",
            scope: "integration",
        ...
        if (isDirectory(candidate))
            state.integrationWorktree = candidate;
        state.result = structuredClone(result);
        this.store.saveState(state);
    ...
    async handleReplan(current, review) {
        const planner = agentForRole(this.config, "planner");
        const [raw] = await this.runner.run({
            agent: planner,
            taskId: "PLAN",
        ...
        this.store.replacePlan(replanned, planner.identity, review.issues.map((issue) => issue.description).join("; "));
        const state = this.store.loadState();
        state.status = "running";
        state.result = null;
        this.store.saveState(state);
    }
    async runValidation(command, cwd) {
    ...
    terminalResult(plan) {
        ...
        const affected = failed.length > 0
            ...
            : blocked.length > 0
                ? blocked
                : plan.tasks.filter((task) => !["approved", "completed", "invalidated"].includes(task.status));
        const issues = affected.map((task) => ({
            severity: "high",
            scope: "task",
    ...
    static task(plan, taskId) {
        const task = plan.tasks.find((candidate) => candidate.id === taskId);
        if (!task)
            throw new SchemaError(`unknown task: ${taskId}`);
        return task;
    }
}
