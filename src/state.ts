import { SchemaError, TaskStatuses, type Task } from "./models.js";
import type { RunStore } from "./persistence.js";

export const SatisfiedDependencyStatuses = new Set(["approved", "completed"]);

export const AllowedTransitions: Record<Task["status"], Set<Task["status"]>> = {
...
export function readyTaskIds(tasks: Task[]): string[] {
    ...
    .filter(
      (task) =>
        ...
        task.dependencies.every((dependency) =>
          SatisfiedDependencyStatuses.has(byId.get(dependency)!.status),
        ),
    )
    .map((task) => task.id)
    .sort();
...
export function transitionTask(
  ...
  if (!(TaskStatuses as readonly string[]).includes(targetStatus)) {
